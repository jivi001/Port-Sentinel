  ...
  return (
    <div className="page-container h-full w-full flex flex-col">
      <header className="page-header flex justify-between items-center mb-4 shrink-0">
        <h1 className="page-title text-3xl font-bold text-white tracking-tight">Console Configuration</h1>
        {toast && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 min-h-0">
        {/* System Health */}
        <section className="bg-surface border border-gray-800 rounded-xl shadow-lg flex flex-col overflow-hidden">
          <div className="p-4 bg-gray-900 border-b border-gray-800">
            <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Operational Health</h2>
          </div>
          <div className="p-6 flex flex-col gap-5 flex-1 overflow-y-auto">
            <div className="flex justify-between items-center pb-4 border-b border-gray-800">
              <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">System Kernel</span>
              <span className="font-mono text-sm font-bold text-accent">{health?.platform?.toUpperCase() || 'SEARCHING...'}</span>
            </div>
            <div className="flex justify-between items-center pb-4 border-b border-gray-800">
              <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">Sniffer Status</span>
            ...
            <div className="flex justify-between items-center pb-4 border-b border-gray-800">
              <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">System Uptime</span>
              <span className="font-mono text-sm font-bold text-white">{health ? `${Math.floor(health.uptime_seconds / 60)}M` : '0M'}</span>
            </div>
          </div>
        </section>
        ...
        <section className="bg-surface border border-gray-800 rounded-xl shadow-lg flex flex-col overflow-hidden">
          <div className="p-4 bg-gray-900 border-b border-gray-800">
            <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Firewall Policy (Hard Blocks)</h2>
          </div>
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="flex-1 overflow-y-auto">
              {blockedPorts.length === 0 ? (
                <div className="p-16 text-center text-gray-500 text-sm font-bold tracking-wider">NO ACTIVE BLOCKS</div>
              ) : (
                blockedPorts.map(p => (
                  <div key={p.port} className="flex items-center p-4 border-b border-gray-800 hover:bg-gray-800/50 transition-colors gap-4">
                    <div className="w-24 font-mono text-danger font-semibold">{p.port}</div>
                    <div className="flex-1 text-xs text-gray-400 truncate">{p.reason || 'MANUAL_BLOCK'}</div>
